...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		public ArrayList<KeyValuePair<K, V>> inRangeValues(K key1, K key2) {
            ...
            MapEntry<K, V> element = new MapEntry<K,V>(key1, this.get(key1));
			...
			if(key1.equals(key2)){ 
				...
				AuxRange(this.root, key1, key2, L);
                return L;
            }		
		}

		public void AuxRange(BinaryTreeNode<MapEntry<K, V>> root, K key1, K key2, ArrayList<KeyValuePair<K, V>> L){
			
			if(root == null){
				return;
			}

			boolean mayor_igual = this.keyComparator.compare(root.getValue().getKey(), key1) >= 0;
			boolean menor = this.keyComparator.compare(root.getValue().getKey(), key2) < 0;

			if( mayor_igual && menor){
				L.add(root.getValue());
			}

			AuxRange(root.getLeftChild(), key1, key2, L);
			AuxRange(root.getRightChild(), key1, key2, L);
