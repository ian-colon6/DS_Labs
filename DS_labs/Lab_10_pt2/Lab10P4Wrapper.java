...
public class Lab10P4Wrapper {
	...
	public static class HashTableSC<K, V> implements Map<K, V> {
		...
		public void put(K key, V value) {
			...
			if( (currentSize/this.buckets.length)*1.0 >= loadFactor){
				System.out.print("REHASHING NEEDED\n");
				this.rehash();
			}
		...
		private void rehash() {
			...
			int rehashed_sz = (this.buckets.length)*2;

			//while( !isPrime(rehashed_sz) ){ rehashed_sz++; }
