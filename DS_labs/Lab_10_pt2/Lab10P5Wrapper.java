...
public class Lab10P5Wrapper {
	...
	public static class HashTableSC<K, V> implements Map<K, V> {
		...
		public void put(K key, V value) {
			...
			if( (this.currentSize*1.0)/this.buckets.length > loadFactor){
		...
		private void rehash() {
            ...
            int rehashed_sz = (this.buckets.length)*2 + 1;

			List<BucketNode<K,V>>[] neo_bucks = new SinglyLinkedList[rehashed_sz];

			for (int i = 0; i < rehashed_sz; i++){
				neo_bucks[i] = new SinglyLinkedList<BucketNode<K, V>>();
			}

			for(int i = 0; i < this.buckets.length; i++){

				for( BucketNode<K, V> BN : buckets[i] ){
					int neo_targetBucket = this.hashFunction.hashCode(BN.getKey()) % neo_bucks.length;
					
					List<BucketNode<K, V>> neo_L = neo_bucks[neo_targetBucket];
					
					neo_L.add(0, new BucketNode<K, V>(BN.getKey(), BN.getValue()));
				}
			}

			this.buckets = neo_bucks;
	...
	public static class HashSet<E> implements Set<E> {
		...
		private SimpleHashFunction<E> function = new SimpleHashFunction<>();
		
		/*Constructs an empty HashSet with a given initial capacity*/
		public HashSet(int initialCapacity) {
			//TODO ADD YOUR CODE HERE
			hashtable = new HashTableSC<E, Object>(initialCapacity, function);
		}
		
		/*Constructs an empty HashSet with a default initial capacity of ten*/
		public HashSet() {
			//TODO ADD YOUR CODE HERE
			hashtable = new HashTableSC<E, Object>(DEFAULT_SET_SIZE, function);
		}
		...
		public boolean add(E obj) {
			//TODO ADD YOUR CODE HERE
			if(this.hashtable.containsKey(obj)){ return false; }

			else{
				this.hashtable.put(obj, 0);
				return true;
			}
		}
		...
		public Set<E> union(Set<E> S2) {
			//TODO ADD YOUR CODE HERE
			int uni_size = this.size() + S2.size();
			Set<E> uni_result = new HashSet<E>(uni_size);

			for(E element : this){
				uni_result.add(element);
			}
			
			for(E element2 : S2){
				uni_result.add(element2);
			}
		...
		public Set<E> difference(Set<E> S2) {
			//TODO ADD YOUR CODE HERE
			int diff_size = this.size() + S2.size();
			Set<E> diff_result = new HashSet<E>(diff_size);

			for(E element : this){

				if(!S2.isMember(element)){
					diff_result.add(element);
				}
			}
		...
		public Set<E> intersection(Set<E> S2) {
			//TODO ADD YOUR CODE HERE
			//int inter_size = this.size() + S2.size();
			Set<E> inter_result = new HashSet<E>();

			for(E element : this){

				if(S2.isMember(element)){
					inter_result.add(element);
				}
			}
		...
		public boolean isSubSet(Set<E> S2) {
			...
			Iterator<E> i2 = S2.iterator();
			while(i2.hasNext()){
				
				if(this.isMember(i2.next())){
					flag = true;
				}else{
					flag = false;
				}
	...
	public static void main(String[] args) {
		Lab10P5Wrapper.Set<Integer> hashSet = new Lab10P5Wrapper.HashSet<>(2);
		Lab10P5Wrapper.Set<Integer> hashSet2 = new Lab10P5Wrapper.HashSet<>(5);
		Lab10P5Wrapper.Set<Integer> hashSet3 = new Lab10P5Wrapper.HashSet<>(5);
		...
		Lab10P5Wrapper.printSet(hashSet); //Test Iterator
		...
		System.out.println("hashSet1: ");
		Lab10P5Wrapper.printSet(hashSet);
		...
		Lab10P5Wrapper.printSet(hashSet2);
		...
		Lab10P5Wrapper.printSet(hashSet3);

		System.out.println("------------------------");

		System.out.println("Union of hashSet and hashSet2 is: \n");
		Lab10P5Wrapper.Set<Integer> union = hashSet.union(hashSet2); //Should be equal
		System.out.println();
		Lab10P5Wrapper.printSet(union);

		System.out.println("------------------------");

		System.out.println("Union of hashSet2 and hashSet3 is: \n");
		Lab10P5Wrapper.Set<Integer> union2 = hashSet2.union(hashSet3); //Should be 1,2,3,4,5,7
		System.out.println();
		Lab10P5Wrapper.printSet(union2);

		System.out.println("------------------------");

		System.out.println("Difference of hashSet and hashSet2 is: ");
		Lab10P5Wrapper.Set<Integer> difference = hashSet.difference(hashSet2); //Should be empty
		System.out.println();
		Lab10P5Wrapper.printSet(difference);

		System.out.println("------------------------");

		System.out.println("Difference of hashSet2 and hashSet3 is: \n");
		Lab10P5Wrapper.Set<Integer> difference2 = hashSet2.difference(hashSet3); //Should be 5,2
		System.out.println();
		Lab10P5Wrapper.printSet(difference2);

		System.out.println("------------------------");

		System.out.println("Intersection of hashSet and hashSet2 is: \n");
		Lab10P5Wrapper.Set<Integer> intersection = hashSet.intersection(hashSet2); //Should be equal
		System.out.println();
		Lab10P5Wrapper.printSet(intersection);

		System.out.println("------------------------");

		System.out.println("Intersection of hashSet and hashSet3 is: \n");
		Lab10P5Wrapper.Set<Integer> intersection2 = hashSet.intersection(hashSet3); //Should be 1,3,4
		System.out.println();
		Lab10P5Wrapper.printSet(intersection2);
