	...
	public static class DoublyLinkedList<E> implements List<E> {
		...
		public void reverse() {
			...
			while(curr != null){
				...
				nxt = curr.getNext();
				prev = curr.getPrev();
				//Reverses the current node's pointers
				curr.setNext(prev);
				curr.setPrev(nxt);
				//Updates the position of previous, current, and next nodes
				prev = curr;
				curr = nxt;
				//Changes header and trailer's pointers to the appropriate elements
				this.header.setNext(prev);
				this.trailer.setPrev(nxt);
