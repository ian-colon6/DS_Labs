...
public class Mock1QueueWrapper {
	...
	public static int countStudents(int[] students, int[] sandwiches) {
        ...
        SinglyLinkedStack<Integer> sands = new SinglyLinkedStack<Integer>();
        DoublyLinkedQueue<Integer> estudiantes = new DoublyLinkedQueue<Integer>();
        int stus = 0;
        int count = 0;

        for(int i = 0; i < students.length; i++){
            estudiantes.enqueue(students[i]);
        }
        ...
        while( !sands.isEmpty() ){
            ...
            if(estudiantes.getFront() == sands.top()){
                estudiantes.dequeue();
                sands.pop();
                stus--;
            }else{
                estudiantes.enqueue(estudiantes.dequeue());
                stus++;
            }

            if(stus > estudiantes.size() ) break;
        }
    ...
    public static void main(String[] args) {
        ...
        int hambrientos = countStudents(studs, sands);

        System.out.println(hambrientos);
    }
}